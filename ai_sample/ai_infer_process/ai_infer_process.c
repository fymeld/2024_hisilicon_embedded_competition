...
char classname[80][20] = { {"person"}, {"bicycle"}, {"car"}, {"motorcycle"}, {"airplane"}, {"bus"}, {"train"}, {"truck"}, {"boat"}, {"traffic light"},
    ...
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, SAMPLE_SVP_NNIE_YOLOV2_SOFTWARE_PARAM_S* pstSoftWareParam)
{
    ...
    pstSoftWareParam->u32BboxNumEachGrid = 3; // 5: 2BboxNumEachGrid
    pstSoftWareParam->u32ClassNum = 80; // 5: class number
    pstSoftWareParam->u32GridNumHeight = 32; // 12: GridNumHeight
    pstSoftWareParam->u32GridNumWidth = 32; // 20: GridNumWidth
...
int Yolo2Create(SAMPLE_SVP_NNIE_CFG_S **model, const char* modelFile)
{
    ...
    SAMPLE_SVP_CHECK_EXPR_GOTO(HI_SUCCESS != s32Ret, YOLOV2_FAIL_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,SampleSvpNnieYolov5ParamInit failed!\n");
...
static unsigned int yolo_result_process(SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam, float *strides, anchor_w_h (*anchor_grids)[3], int *map_size, yolo_result **output_result, float confidence_threshold)
{
	...
	*output_result = NULL;
    ...
    SAMPLE_SVP_TRACE_INFO("start Yolo result process!\n");
    SAMPLE_PRT("yolo parameter={ strides= %.2f %.2f %.2f, mapsize= %d %d %d}\n",strides[0],strides[1],strides[2],map_size[0],map_size[1],map_size[2]);
    SAMPLE_PRT("feature length is %d,output num is %d,anchor num is %d\n",
            pstNnieParam->astSegData[0].astDst[0].unShape.stWhc.u32Width,
            pstNnieParam->astSegData[0].astDst[0].unShape.stWhc.u32Height,
            pstNnieParam->astSegData[0].astDst[0].unShape.stWhc.u32Chn);
    ...
    for (int yolo_layer_index = 0; yolo_layer_index < yolo_layer_num; yolo_layer_index ++) { // 3 yolo layer 
		...
		feature_length = pstNnieParam->astSegData[0].astDst[yolo_layer_index].unShape.stWhc.u32Width; // 6400 / 1600 / 400
		
		output_num = pstNnieParam->astSegData[0].astDst[yolo_layer_index].unShape.stWhc.u32Height; // 85
		...
		for (int anchor_index = 0; anchor_index < anchor_num; anchor_index ++){ // 每个 grid 上有三个 anchor
				...
				for (int coord_x_y = 0; coord_x_y < feature_length; coord_x_y ++){ // feature size 拉直后的长度，如 6400 1600 400.
					...
					//printf("output confidence is %.2f\n",confidence);
					if (confidence > confidence_threshold){
						...
						for (int output_index = 5; output_index < output_num; output_index ++){
							...
							// 注意，yolo v5 的类别置信度并不需要选出个最大值，它的 label 是多标签，所以并不是 softmax，我在博客里说明一下，输出多个类别
...
//返回值为检测到目标数量，yolo_result存储yolov5检测到的目标信息，resBuf为输出的目标信息
int printf_result(yolo_result *temp,int resSize,DetectObjInfo resBuf[]){
	printf("--------------------\n");
    
    memset_s(resBuf, resSize * sizeof(resBuf[0]), 0x00, resSize * sizeof(resBuf[0]));
    int i = 0;
	while (temp != NULL && i < resSize){
		
        if(temp->class_index <= 6){
            resBuf[i].box.xmin = (int)temp->left_up_x;
            resBuf[i].box.ymin = (int)temp->left_up_y;
            resBuf[i].box.xmax = (int)temp->right_down_x;
            resBuf[i].box.ymax = (int)temp->right_down_y;
            resBuf[i].score = temp->score;
            resBuf[i].cls = temp->class_index;
		    printf("box.xmin= %d\t",resBuf[i].box.xmin);
		    printf("box.ymin= %d\n",resBuf[i].box.ymin);

		    printf("box.xmax= %d\t",resBuf[i].box.xmax);
		    printf("box.ymax= %d\n",resBuf[i].box.ymax);

		    printf("class = %s\t", classname[temp->class_index]);
		    printf("score = %f\n\n", temp->score);
		
		    temp = temp->next;
            i++;

        }else if(temp->class_index == 7 || temp->class_index == 9 || temp->class_index == 11){
            resBuf[i].box.xmin = (int)temp->left_up_x;
            resBuf[i].box.ymin = (int)temp->left_up_y;
            resBuf[i].box.xmax = (int)temp->right_down_x;
            resBuf[i].box.ymax = (int)temp->right_down_y;
		    ...
		    temp = temp->next;
            i++;
        }else if(temp->class_index >= 15 && temp->class_index <= 23){
            resBuf[i].box.xmin = (int)temp->left_up_x;
            resBuf[i].box.ymin = (int)temp->left_up_y;
            resBuf[i].box.xmax = (int)temp->right_down_x;
            resBuf[i].box.ymax = (int)temp->right_down_y;
    ...
    const IVE_IMAGE_S *img, DetectObjInfo resBuf[], int resSize, int* resLen)
{
    ...
    *resLen = 0; 
     ...
     * function's input datas are correct
     */
    unsigned int result = yolo_result_process(&g_stYolov2NnieParam, strides, anchor_grids, map_size, &output_result, CONF_THRE); // 后处理
    SAMPLE_SVP_CHECK_EXPR_GOTO( result != 0, YOLOV2_FAIL_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error,yolo result process failed!\n");
    if(output_result != NULL){
		yolo_result_sort(output_result); // 输出结果排序，方便下面的 nms 处理

        //printf_result(output_result);
	
	    yolo_nms(output_result, IOU_THRES); // nms
	}

    *resLen=printf_result(output_result,resSize,resBuf); // 打印结果看下，这里打印的已经是可以输出画框的结果了
