...
static HI_VOID ObjectDetectAiProcess(VIDEO_FRAME_INFO_S frm, VO_LAYER voLayer, VO_CHN voChn)
{
    int ret;
    if (g_workPlug.model == 0) {
        printf("g_workPlug.model = %d\n",g_workPlug.model);
        ret = YOLOV5ModelLoad();
        g_workPlug.model = 0xff;
        if (ret < 0) {
            g_workPlug.model = 0;
            SAMPLE_CHECK_EXPR_GOTO(ret < 0, HAND_RELEASE,
                "load YOLOV5 model err, ret=%#x\n", ret);
        }else{
            SAMPLE_PRT("YOLOV5 Model Load Success!");
        }
    }
    VIDEO_FRAME_INFO_S resizeFrm;
    ret = MppFrmResize(&frm, &resizeFrm, HAND_FRM_WIDTH, HAND_FRM_HEIGHT);
    printf("resize frame width=%d,height=%d\n",resizeFrm.stVFrame.u32Width,resizeFrm.stVFrame.u32Height);
    ret = YOLOV5ObjectDetectCal(&resizeFrm, &frm);
    SAMPLE_CHECK_EXPR_GOTO(ret < 0, HAND_RELEASE,
        "hand classify plug cal FAIL, ret=%#x\n", ret);
...
static HI_VOID* GetVpssChnFrameObjectDetect(HI_VOID* arg)
{
    int ret;
    VIDEO_FRAME_INFO_S frm;
    HI_S32 s32MilliSec = 2000;
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
        ...
        ObjectDetectAiProcess(frm, voLayer, voChn);
    }
...
static HI_S32 ObjectDetectionThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "AIProcess") < 0) {
        HI_ASSERT(0);
    ...
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameObjectDetect, NULL);
...
HI_S32 SAMPLE_MEDIA_Detection(HI_VOID)
{
    HI_S32             s32Ret;
    HI_S32             fd = 0;
     ...
     * Create work thread to run ai
     */
    s32Ret = ObjectDetectionThreadProcess();
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "ai proccess thread creat fail:%s\n", strerror(s32Ret));
